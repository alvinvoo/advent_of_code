...
;(#{not (equal? 'quote %)} 'quote)
;; above expands to
;((lambda (x) (not (equal? 'quote x))) 'quote)

  ;(define in-syn (syntax->datum stx))
  ;(define var (list-ref in-syn 1))
  ;(define params-list (cddr in-syn))

  ;(displayln (append (list 'vector-set*! var) params-list))
  
  ;(datum->syntax stx
  ;  (append (list 'vector-set*! var) params-list))
  ...
  (cond 
    ...
    [else
      (let ([start-row (first start-pos)]
        ...
        (match-define (list _ dir step-s-b) 
          (regexp-match #rx"^([RLUD])(.*)" cur-wire-dir))
